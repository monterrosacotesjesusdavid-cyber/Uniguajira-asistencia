...
router.get('/dashboard', verificarToken, verificarAdmin, async (req, res) => {
  try {
    ...
    const diasES = ['Domingo','Lunes','Martes','Miércoles','Jueves','Viernes','Sábado']; const diaHoy = diasES[new Date().getDay()]; const [[{ total_clases_hoy }]] = await db.query('SELECT COUNT(*) AS total_clases_hoy FROM horarios WHERE dia_semana = ? AND activo = TRUE', [diaHoy]);
    const ausentes_hoy = Math.max(0, total_clases_hoy - presentes_hoy); res.json({ total_profesores, presentes_hoy, tardanzas_hoy, total_clases_hoy, ausentes_hoy });
  } catch (err) {
